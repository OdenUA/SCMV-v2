...
function renderVehicleTable() {
  ...
  // Add Action column for per-row buttons (selection) only for non-ТС overlays
  ...
  if (deviceListBtn && !deviceListBtn.dataset.bound) {
    ...
    deviceListBtn.addEventListener('click', function(){
      ...
      // ensure Device List uses list mode (not edit mode) so Action shows 'Выбрать'
...
function applyVehicleFilters() {
  ...
  // Apply color/category filters only for 'selectMin' overlay (ТС). Device List ('show') should not be filtered by these buttons.
  try {
    if (vehicleOverlayMode === 'selectMin') {
  ...
  if (
    ...
    setTimeout(function () {
      ...
      try {
        ...
        if (vehicleOverlayMode === 'selectMin') {
          // Request minimal select list (ТС)
