...
- **Управление памятью** ⚠️ **[ВАЖНО]**
   - Автоматическая очистка данных предыдущего устройства при переключении устройств
   - Активные данные (текущий трек и таблицы) автоматически не удаляются
   - Кнопка "🧹 Очистить память" для принудительной очистки
   - Мониторинг лимитов каждые 30 секунд: только статистика в консоль
