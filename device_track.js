// Восстановить функционал: при клике на трек показывать время ближайшей точки
function attachRouteAwareClick(poly) {
  if (!poly || !poly._dtPoints || !Array.isArray(poly._dtPoints)) return;
  poly.on('click', function(ev) {
    // If route mode active, consume the click and convert it into a route point
    if (routeModeActive) {
...
function drawRawDeviceTrack(points){
  ...
  mEnd.on('click', function(ev){ if (routeModeActive){ var ll = ev && ev.latlng?ev.latlng:mEnd.getLatLng(); if(ll && typeof onRouteMapClick === 'function') onRouteMapClick({ latlng: ll }); if(ev && ev.originalEvent && ev.originalEvent.stopPropagation) ev.originalEvent.stopPropagation(); } else { try{ mEnd.openPopup(); }catch(_){} } });
    ...
    // we can inject the "Показать в Full" button into existing popups.
...
// Форматирует дату для таблицы аномалий: DD.MM.YY HH:mm:ss
...
(function(){
  ...
  var _fullTrackCache = null; // сохраняем последнюю полную выборку
  var _fullIntervals = []; // кэш интервалов после фильтрации
  ...
  window._awaitingFullTrackSetup = true; // помечаем ожидание ответа
  ...
  try { if(fullHead && fullBody){ window.__dt_setTableLoading('fullDeviceTrackTable', true); } } catch(e){}
        ...
        for(var si=0; si<segs.length; si++){
          try{
            ...
            var segReq = { name:'Device Track', type:'etbl', mid:6, act:'setup', filter:[ {selectedpgdateto:[toIso]}, {selectedpgdatefrom:[fromIso]}, {selecteddeviceid:[deviceIdInput.value]} ], nowait:false, waitfor:['selectedpgdateto'], usr:authUser, pwd:authPwd, uid:authUid, lang:'en' };
  ...
  // Экспортируем функцию для внешнего вызова (например, по нажатию Dev Log)
  ...
  window.__handleFullTrackSetup = function(data){
    ...
    if(!window._awaitingFullTrackSetup) return false; // не наш ответ
    if(data.res && data.res[0] && Array.isArray(data.res[0].f)){
      ...
      if(window._awaitingFullTrackSetupSegments && typeof window._awaitingFullTrackSetupSegments === 'object'){
        ...
        // Автоматический запуск отчёта по полной выборке
        try { saveFullTrackSettings(); runReport(); } catch(e) { console.warn('Авто отчёт не выполнен', e); }
  try { window.__dt_setTableLoading('fullDeviceTrackTable', false); } catch(e){}
        return true;
      }
  ...
  function buildIntervals(data){
    ...
    var norm = data.map(function(r, idx){
      return {
        idx: idx, // индекс строки для подсветки
  ...
  if(typeof satThresholdInput !== 'undefined' && satThresholdInput){ var stv = parseInt(satThresholdInput.value,10); if(!isNaN(stv) && stv>0) satThreshold = stv; }
    function classify(n){
      ...
      var type1 = ign && move;          // условие 1
      var type2 = (sats!=null && sats < satThreshold); // условие 2 (динамический порог)
      var type = 0;
  ...
  var headers = ['#','Type','Period','Duration','Distance (km)','Rows'];
    ...
    var fullTableBody = fullBody; // для подсветки строк
    // Снимаем прежние подсветки и фокус
    if(fullTableBody){
