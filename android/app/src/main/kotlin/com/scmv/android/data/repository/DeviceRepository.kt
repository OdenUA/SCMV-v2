...
class DeviceRepositoryImpl @Inject constructor(
    ...
    override suspend fun getDevices(): Result<List<Device>> {
        return try {
            ...
            // Step 1: Send init request and wait for response to get fleet mappings
            ...
            Log.d(TAG, "Sending init request: $initRequestJson")
            ...
            Log.d(TAG, "Init response received - name: ${initResponse.name}, res size: ${initResponse.res?.size}")
            Log.d(TAG, "Init response structure: ${json.encodeToString(initResponse)}")

            // Step 2: Extract fleet map from init response cols
            val fleetMap = buildFleetMapFromResponse(initResponse)
            Log.d(TAG, "Fleet map extracted - size: ${fleetMap.size}, entries: $fleetMap")

            // Step 3: Send setup request and wait for response with device data
            ...
            Log.d(TAG, "Sending setup request: $setupRequestJson")
            ...
            Log.d(TAG, "Setup response received - name: ${setupResponse.name}, res size: ${setupResponse.res?.size}")
            Log.d(TAG, "Setup response structure: ${json.encodeToString(setupResponse)}")

            // Step 4: Parse devices using the fleet map from init response
            val devices = parseDevicesResponse(setupResponse, fleetMap)
            Log.d(TAG, "Total devices parsed: ${devices.size}")
